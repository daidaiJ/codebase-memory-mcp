...
| `CBM_DACL_HARDENING` | *(unset = skip)* | Windows only, fork patch: the cache-directory untrusted-ACE walk (the "cache-private / DACL entry grants mutation rights to untrusted identity" startup refusal) is **skipped by default** — ancestor directories outside the user's control (shared tool dirs inheriting an Authenticated Users ACE) must not block startup of a single-user local tool. Owner validation stays active in both modes. Set `=1` to opt back into the strict walk on multi-user or terminal-server hosts, where it protects the cache from other local accounts. Env-only by design: the config store lives inside the cache directory, so a config key cannot affect the first run that creates it. |
